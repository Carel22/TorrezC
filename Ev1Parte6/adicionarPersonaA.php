...
if (isset($_POST['id_Persona'], $_POST['nombre'],$_POST['apellido'],  $_POST['email'], $_POST['rol'], $_POST['ciudad'], $_POST['edad'], $_POST['username'], $_POST['password'])) {
...
// Verificar si el id_Ci ya existe en la tabla 
    ...
    if ($count == 0) {
        // El id_Ci no existe, proceder con la inserción en Responsable
         $sql = "INSERT INTO persona (id_Persona, nombre, apellido, email, rol, ciudad, edad, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($con, $sql);
        if ($stmt) 
        {
            mysqli_stmt_bind_param($stmt, "sssss", $id_Persona, $nombre, $apellido, $email, $rol, $ciudad, $edad, $username, $password);
