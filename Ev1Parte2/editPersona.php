...
mysqli_stmt_close($stmt_persona_verificar);
    ...
    if ($persona_count >= 0  ) {
        ...
        $sql_update = "UPDATE  persona SET nombre=?, apellido=?, email=?, rol=?, ciudad=?, username=?, password=?, edad=? WHERE id_Persona=?";
        $stmt_update = mysqli_prepare($con, $sql_update);

       // Vincular los parámetros

            mysqli_stmt_bind_param($stmt_update, "sssssssss", $nombre, $apellido, $email, $rol, $ciudad, $username, $password, $edad, $id);
